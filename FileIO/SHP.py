import shapefile # pyshp.__version__ == 1.2.12
...
TYPES_SHP = {'float':'N', "<class 'float'>": 'N', 'int': 'N', 'str':'C', \
             'string':'C', 'bool':'L'}
...
class Point():
    ...
    def new(shpPath, fields, records, geometryKey):
        ...
        for col in fields:
            if col[1] in TYPES_SHP.keys():
                if col[1] == 'float' or col[1] == "<class 'float'>":
                    point_shp.field(col[0][:10], TYPES_SHP[col[1]], decimal=8)
                else:
                    point_shp.field(col[0][:10], TYPES_SHP[col[1]])
            else:
...
class Polyline():
    ...
    def new(shpPath, polylineGraph, fields, records, geometryKey):
        ...
        for col in fields:
            if col[1] in TYPES_SHP.keys():
                if col[1] == 'float' or col[1] == "<class 'float'>":
                    polyline_shp.field(col[0][:10], TYPES_SHP[col[1]], decimal=8)
                else:
                    polyline_shp.field(col[0][:10], TYPES_SHP[col[1]])
            else:
    ...
    def fromGraph(polylineGraph, shpPath):
        ...
        shpPath:        string
                        path of shapefile to handle
        """
        polylineGraph_copy = copy.deepcopy(polylineGraph)
        
        colList = [('X1', 'float'), ('Y1', 'float'), ('X2', 'float'), ('Y2', 'float')]
        for key, val in polylineGraph_copy.edges(data=True)[0][2].items():
            colList.append((key, str(type(val))))
        
        recordList = []
        for edge in polylineGraph_copy.edges(data=True):
            rowList = [edge[0][0], edge[0][1], edge[1][0], edge[1][1]]
            for col in colList[4:]:
                rowList.append(edge[2][col[0]])
            recordList.append(rowList)
            
        Polyline.new(shpPath, polylineGraph_copy, colList, recordList, (0, 1, 2, 3))
...
class Projection():
    ...
    def getWKT_PRJ(crsCode, crsType):
        ...
        """
        crsTypeDict = {'epsg':1, 'esri':2, 'sr-org':3}
        i = crsTypeDict[crsType.lower()]
        if i == 1:
            # access projection information
            wkt = urllib.request.urlopen("http://spatialreference.org/ref/epsg/{0}/prettywkt/".format(crsCode))
        elif i== 2:
            wkt = urllib.request.urlopen("http://spatialreference.org/ref/esri/{0}/prettywkt/".format(crsCode))
        elif i == 3:
    ...
    def getPROJ4(crsCode, crsType):
        ...
        if i == 1:
            ...
            proj4 = urllib.request.urlopen("http://spatialreference.org/ref/esri/{0}/proj4/".format(crsCode))
        elif i == 3:
